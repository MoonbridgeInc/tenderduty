...
| `chain."name".alerts.stalled_minutes`      | How long a halted chain takes in minutes to generate an alarm. Also reused as the per-node threshold for how far behind the chain head an individual RPC node must fall before it is considered lagging.                                                                                                                                                                        |
...
| `chain."name".alerts.double_sign_alerts`   | Should an alert be sent if tendermint reports double-signing (DuplicateVoteEvidence) against this validator? This is a critical, near-instant tombstone/slashing event.                                                                                                                                                                                                           |
...
| `chain."name".nodes[].alert_if_down` | Should an alert be sent if this host isn't responding, or has fallen behind the chain head (lagging)? Uses the `node_down_alert_minutes` setting to determine threshold.    |
